...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        cli::Command::Index(args) => {
            ...
            let output =
                crate::store::save_project(&args.project, args.output.as_deref(), &graph)?;
            println!(
                "{}",
                serde_json::json!({
                    "kind": "index",
        ...
        // ---- Query ----
        cli::Command::Query(cmd) => {
            match cmd {
                cli::QueryCmd::Inspect(args) => {
                    let graph = crate::store::load(args.graph.as_deref())?;
                    let result =
                        crate::query::inspect(&graph, &args.name, !args.no_source)?;
                    println!("{}", serde_json::to_string_pretty(&result)?);
                }
                cli::QueryCmd::Trace(args) => {
                    ...
                    let result = crate::query::trace(
                        &graph, &args.name, direction, args.depth, args.limit,
                    )?;
                    println!("{}", serde_json::to_string_pretty(&result)?);
                }
                ...
                cli::QueryCmd::Path(args) => {
                    let graph = crate::store::load(args.graph.as_deref())?;
                    let result =
                        crate::query::shortest_path(&graph, &args.from, &args.to, args.depth);
                    println!("{}", serde_json::to_string_pretty(&result)?);
                }
                cli::QueryCmd::Symbols(args) => {
                    ...
                    let result = crate::query::symbols(
                        &graph,
                        args.kind.as_deref(),
                        args.limit,
                        filter,
                ...
                cli::QueryCmd::Impact(args) => {
                    let graph = crate::store::load(args.graph.as_deref())?;
                    let result =
                        crate::query::impact(&graph, &args.name, args.depth, args.limit)?;
                    println!("{}", serde_json::to_string_pretty(&result)?);
                }
            }
        }
        ...
        cli::Command::Nav(cmd) => {
            match cmd {
                cli::NavCmd::Map(args) => {
                    let graph = crate::store::load(args.graph.as_deref())?;
                    let result =
                        crate::nav::nav_map(&graph, args.full, args.budget)?;
                    println!("{}", serde_json::to_string_pretty(&result)?);
                }
                cli::NavCmd::Ask(args) => {
                    let graph = crate::store::load(args.graph.as_deref())?;
        ...
        // ---- Analyze ----
        cli::Command::Analyze(cmd) => {
            match cmd {
                cli::AnalyzeCmd::Deps(args) => {
                    let graph = crate::store::load(args.graph.as_deref())?;
                    let result =
                        crate::analyze::deps(&graph, args.from.as_deref(), args.limit);
                    println!("{}", serde_json::to_string_pretty(&result)?);
                }
